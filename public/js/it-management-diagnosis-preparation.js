...
(() => {
  ...
  const labels = { SURVEY_COMPLETED: 'アンケート回答完了', PREPARATION_IN_PROGRESS: '診断準備中', READY_FOR_DIAGNOSIS: '診断Plan確定', DIAGNOSIS_IN_PROGRESS: '診断中', HUMAN_REVIEW_REQUIRED: 'Human Review待ち', REPORT_REVIEW_REQUIRED: 'Report作成待ち', REPORT_APPROVED: 'Report承認済み', FEEDBACK_PENDING: 'Feedback待ち', FEEDBACK_COMPLETED: 'Feedback完了' };
  ...
  async function load(aiOnly=false) {
    ...
    try {
      ...
      el('workspace-link').hidden=!['READY_FOR_DIAGNOSIS','DIAGNOSIS_IN_PROGRESS','HUMAN_REVIEW_REQUIRED','REPORT_REVIEW_REQUIRED','REPORT_APPROVED','FEEDBACK_PENDING','FEEDBACK_COMPLETED'].includes(next.diagnosis_status);
