...
export interface DeletionExecutionResult {
  ...
  anonymized:{ source_records:number; survey_responses:number; participants:number; ai_executions:number };
...
async function anonymizeGeneralRaw(c:PoolClient,requestId:string,caseId:string,actorId:string){
  ...
  return {source:source.changed,survey,participants,tombstones};
}
...
export class RetentionDeletionWorker {
  ...
  async executeApprovedRequest(caseId:string,requestId:string,executedByUserId:string):Promise<DeletionExecutionResult>{
    ...
    try{
      ...
      if(['COMPLETED','PARTIALLY_RETAINED'].includes(req.status)){
        ...
        return {request_id:requestId,diagnosis_case_id:caseId,status:req.status as 'COMPLETED'|'PARTIALLY_RETAINED',anonymized:{source_records:0,survey_responses:0,participants:0,ai_executions:0},restricted_classes:[],tombstones_written:0,idempotent:true};
      }
      ...
      // Requests that include this class are therefore restricted-retain unless a future, separately reviewed worker implements its exact table semantics.
      if(scoped.has('APPROVED_DECISION_EVIDENCE')) restricted.add('APPROVED_DECISION_EVIDENCE');

      let sourceRecords=0,surveyResponses=0,participants=0,aiExecutions=0,tombstones=0;
      if(scoped.has('GENERAL_RAW_DIAGNOSIS')&&!restricted.has('GENERAL_RAW_DIAGNOSIS')){
        ...
        sourceRecords+=r.source; surveyResponses+=r.survey; participants+=r.participants; tombstones+=r.tombstones;
      }
      ...
      await c.query(`INSERT INTO diagnosis_audit_logs(id,diagnosis_case_id,command,actor_type,actor_user_id,detail_json)
        VALUES($1,$2,'ExecuteDeletionRequest','STAFF',$3,$4)`,[randomUUID(),caseId,executedByUserId,JSON.stringify({deletion_request_id:requestId,status,restricted_classes:restrictedClasses,anonymized:{source_records:sourceRecords,survey_responses:surveyResponses,participants,ai_executions:aiExecutions},tombstones_written:tombstones})]);
      await c.query('COMMIT');
      return {request_id:requestId,diagnosis_case_id:caseId,status,anonymized:{source_records:sourceRecords,survey_responses:surveyResponses,participants,ai_executions:aiExecutions},restricted_classes:restrictedClasses,tombstones_written:tombstones,idempotent:false};
    }catch(error){
  ...
  async previewPolicyExpiry(caseId:string){
    const {rows}=await this.pool.query(`SELECT c.id,c.closed_at,
