...
export async function buildReportContext(c:PoolClient,id:string):Promise<ReportContext>{
 ...
 const {rows:rawInsights}=await c.query<Omit<ReportInsight,'report_text'>>(`SELECT id,version,semantic_type,title,content,unknown_type,area_tag,improvement_lens FROM diagnosis_insights WHERE diagnosis_case_id=$1 AND review_status='HUMAN_APPROVED' ORDER BY created_at,id`,[id]);
 ...
 const insights:ReportInsight[]=rawInsights.map(i=>({...i,report_text:insightReportText(i)}));
 ...
 const observations=insights.filter(i=>i.semantic_type==='OBSERVATION');
 const why_connections=insights.filter(i=>['HYPOTHESIS','ROOT_CAUSE_HYPOTHESIS'].includes(i.semantic_type)).map(hypothesis=>{
  ...
  return {hypothesis_insight_id:hypothesis.id,supporting_insight_refs:supporting.map(i=>i.id),evidence_confirmation_refs:evidence.map(i=>i.id),report_text:whyConnectionReportText(hypothesis,supporting,evidence)};
 });
 const knowledgeStatus=futureKnowledgeStatus(future.statement);
 return {organization_display_name:companyDisplayName(org[0].name),provider_display_name:'atLIB株式会社',future:{...future,knowledge_status:knowledgeStatus,report_text:futureReportText({...future,knowledge_status:knowledgeStatus})},insights,assessment_confirmation_items:items,why_connections};
}
