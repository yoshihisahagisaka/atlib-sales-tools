...
import { futureKnowledgeStatus,futureReportText,insightReportText,type ReportContext,type ReportInsight } from '../domain/diagnosisReport';
...
export async function buildReportContext(c:PoolClient,id:string):Promise<ReportContext>{
 ...
 return {organization_display_name:companyDisplayName(org[0].name),provider_display_name:'atLIB株式会社',future:{...future,knowledge_status:knowledgeStatus,report_text:futureReportText({...future,knowledge_status:knowledgeStatus})},insights:insights.map(i=>({...i,report_text:insightReportText(i)})),assessment_confirmation_items:items.map(a=>({...a,report_text:`Assessmentでの確認候補：${a.title}\n確認目的：${a.purpose}`}))};
}
