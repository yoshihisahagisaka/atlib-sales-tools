...
export class DiagnosisReportRepo {
 ...
 private async transition(c:PoolClient,row:ReportCase,actor:Actor,to:string,command:string,detail:unknown){await c.query('UPDATE diagnosis_cases SET diagnosis_status=$2,closed_at=NULL,closed_by_user_id=NULL WHERE id=$1',[row.id,to]);await c.query(`INSERT INTO case_transitions(id,diagnosis_case_id,from_status,to_status,command,actor_type,actor_user_id) VALUES($1,$2,$3,$4,$5,'STAFF',$6)`,[randomUUID(),row.id,row.diagnosis_status,to,command,this.staff(actor)]);await this.audit(c,row.id,actor,command,detail);}
 ...
 async reissue(id:string,actor:Actor,expectedVersion:number,reason:string){return this.tx(async c=>{
  const row=await this.locked(c,id,actor,['REPORT_APPROVED','FEEDBACK_PENDING','FEEDBACK_COMPLETED','CLOSED'],expectedVersion),previous=await this.latest(c,id);if(!previous||!['APPROVED','DELIVERED'].includes(previous.status))throw new DiagnosisError(409,'再発行対象の承認済Reportがありません。');if(!reason.trim())throw new DiagnosisError(422,'再発行理由が必要です。');
