...
import type { KaizenDiagnosticRepo, ReviewStatus } from '../services/kaizenDiagnosticRepo';
...
/** requireStaffAuthの配下でマウントされる前提（server.ts側で適用）。routes/adminFreeHearingAssessment.ts と同じ構造。 */
export function createAdminKaizenDiagnosticRouter(repo: KaizenDiagnosticRepo): Router {
  const router = Router();
  ...
  router.get('/', async (req, res) => {
    ...
    const { items, total } = await repo.listDiagnostics({
  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      console.error('kaizen diagnostic (staff) insert failed:', err);
