...
function clarificationId(value:unknown):string{
 const parsed=z.string().uuid().safeParse(value);
 if(!parsed.success)throw new DiagnosisError(400,'確認項目IDが無効です。');
 return parsed.data;
}
...
export function createAssessmentScopeClarificationRouter(repo:AssessmentScopeClarificationRepo){
 ...
 r.post(base+'/:clarificationId/resolve',diagnosisHandler(async(req,res)=>{const p=resolveSchema.safeParse(req.body);if(!p.success)throw new DiagnosisError(422,'入力内容を確認してください。');await repo.resolve(caseId(req),clarificationId(req.params.clarificationId),staffActor(req),p.data);res.status(204).end();}));
 r.post(base+'/:clarificationId/not-required',diagnosisHandler(async(req,res)=>{const p=notRequiredSchema.safeParse(req.body);if(!p.success)throw new DiagnosisError(422,'入力内容を確認してください。');await repo.notRequired(caseId(req),clarificationId(req.params.clarificationId),staffActor(req),p.data);res.status(204).end();}));
 return r;
}
