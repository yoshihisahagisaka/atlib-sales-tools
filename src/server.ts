...
async function main(): Promise<void> {
  ...
  const notifySurveyCompleted=async(id:string)=>{const detailUrl=`${config.portalBaseUrl}/admin/it-management-diagnosis-detail.html?id=${id}`;await Promise.all([mailer.sendItManagementSurveyNotification(config.diagnosticNotifyEmail,detailUrl),sendSlackNotification(config.slack.webhookDiagnostic,`【無料 IT経営診断】アンケート回答完了\n提供：atLIB株式会社\n${detailUrl}`)]);};
