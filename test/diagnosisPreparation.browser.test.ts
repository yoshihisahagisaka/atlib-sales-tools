...
test('AI提案→担当者が準備開始→編集採用・質問採用・却下→順序変更→Plan確定',async({page},info)=>{
  ...
  await expect(page.locator('#company')).toHaveText('ABC株式会社様'); await expect(page.locator('#future-status')).toContainText('顧客の回答として記録');
  await page.locator('#run-ai').click();
  await expect(page.locator('#executions')).toContainText('整理完了');
  await expect(page.locator('#case-status')).toContainText('回答完了');
  expect((await h.preparation.read(c.id,operator)).themes).toHaveLength(0);
  await expect(page.locator('#confirm')).toBeDisabled();
  await page.locator('#start').click(); await expect(page.locator('#case-status')).toContainText('診断準備中');
  ...
  await expect(page.locator('#plan-items')).toContainText('資料等の有無を確認する');
  ...
  await expect(page.locator('#plan-items section').first()).toContainText('資料等の有無を確認する');
  ...
  await expect(page.locator('#case-status')).toContainText('確認内容確定');
