...
test('Report: AI Draft→Grounding→wording→Human approval→Delivery→Raw Feedback完了',async({page},info)=>{
 ...
 await page.locator('#run-ai').click();await expect(page.locator('#executions')).toContainText('作成完了');await expect(page.locator('#case-status')).toHaveText('REPORT_REVIEW_REQUIRED');await expect(page.locator('#report-sections > section')).toHaveCount(5);await expect(page.locator('#report-future-status')).toHaveText('SURVEY_STATED');
...
test('Human-only: AI failure→manual→meaning edit rejection→Human Review→regeneration→reissue',async({page},info)=>{
 ...
 await page.locator('#manual').click();await expect(page.locator('#report-status')).toHaveText('第2版 / 担当者確認待ち');await expect(page.locator('#report-sections')).toContainText('担当者と役割の境界は未確認');page.once('dialog',d=>d.accept());await page.locator('#approve').click();await expect(page.locator('#case-status')).toHaveText('REPORT_APPROVED');const old=(await h.report.read(c.id,operator)).reports[0]!;
