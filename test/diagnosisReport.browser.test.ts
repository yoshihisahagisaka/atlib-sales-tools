...
test('Report: AI Draft→Grounding→wording→Human approval→Delivery→Raw Feedback完了',async({page},info)=>{
 ...
 await expect(page.locator('#company')).toHaveText('ABC株式会社様');await expect(page.getByText('IT経営KAIZEN 無料診断 / 経営フィードバック / 提供：atLIB株式会社',{exact:true})).toBeVisible();await expect(page.locator('#tab-assessment')).toBeEnabled();
 await page.locator('#run-ai').click();await expect(page.locator('#executions')).toContainText('作成完了');await expect(page.locator('#case-status')).toContainText('REPORT_REVIEW_REQUIRED');await expect(page.locator('#report-sections > section')).toHaveCount(5);await expect(page.locator('#report-future-status')).toContainText('SURVEY_STATED');
 ...
 page.once('dialog',d=>d.accept());await page.locator('#approve').click();await expect(page.locator('#case-status')).toContainText('REPORT_APPROVED');await expect(page.locator('#approval-info')).toContainText('operator@atlib.jp');await expect(page.locator('#snapshot')).toContainText('approved_by');await expect(page.locator('#approve')).toBeDisabled();await expect(block.getByRole('button',{name:'表現を調整する'})).toBeDisabled();
 ...
 page.once('dialog',d=>d.accept());await page.locator('#deliver').click();await expect(page.locator('#case-status')).toContainText('FEEDBACK_PENDING');const frozen=(await h.report.read(c.id,operator)).reports;
 ...
 page.once('dialog',d=>d.accept());await page.locator('#complete-feedback').click();await expect(page.locator('#case-status')).toContainText('FEEDBACK_COMPLETED');await expect(page.locator('#save-feedback')).toBeDisabled();const result=await h.report.read(c.id,operator);expect(result.feedback[0].content).toBe(raw);expect(result.reports).toEqual(frozen);expect(errors).toEqual([]);await page.screenshot({path:info.outputPath('feedback-completed.png'),fullPage:true});
...
test('Human-only: AI failure→manual→meaning edit rejection→Human Review→regeneration→reissue',async({page},info)=>{
 ...
 await page.locator('.report-block').filter({hasText:'まだ分かっていないこと（UNRESOLVED）'}).getByRole('button',{name:'表現を調整する'}).click();await page.locator('#wording-text').fill('原因は担当者の不在です');await page.getByRole('button',{name:'表現を保存',exact:true}).click();await expect(page.locator('#report-error')).toContainText('内容の意味が変わる編集は、この画面では保存できません');await page.locator('#cancel-wording').click();
 await page.locator('#revision-reason').fill('診断の意味をHuman Reviewで確認');await page.locator('#return-review').check();await page.locator('#request-revision').click();await expect(page.locator('#case-status')).toContainText('HUMAN_REVIEW_REQUIRED');await expect(page.locator('#approve')).toBeDisabled();await page.locator('#review-link').click();
 await page.locator('#approved').getByRole('button',{name:'内容を更新する',exact:true}).first().click();await page.locator('#insight-content').fill('担当者と役割の境界は未確認');await page.locator('#review-reason').fill('未確認範囲を明確化');await page.locator('#save-insight').click();await expect(page.locator('#approved')).toContainText('担当者と役割の境界は未確認');page.once('dialog',d=>d.accept());await page.locator('#complete').click();await expect(page.locator('#case-status')).toContainText('REPORT_REVIEW_REQUIRED');await page.locator('#report-link').click();
 await page.locator('#manual').click();await expect(page.locator('#report-status')).toHaveText('第2版 / 担当者確認待ち');await expect(page.locator('#report-sections')).toContainText('担当者と役割の境界は未確認');page.once('dialog',d=>d.accept());await page.locator('#approve').click();await expect(page.locator('#case-status')).toContainText('REPORT_APPROVED');const old=(await h.report.read(c.id,operator)).reports[0]!;
