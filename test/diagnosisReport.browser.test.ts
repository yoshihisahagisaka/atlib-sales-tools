...
test('Report: AI Draft→Grounding→wording→Human approval→Delivery→Raw Feedback完了',async({page},info)=>{
 ...
 await expect(page.locator('#company')).toHaveText('ABC株式会社様');await expect(page.getByText('無料 IT経営診断 / 提供：atLIB株式会社',{exact:true})).toBeVisible();await expect(page.locator('#tab-assessment')).toBeDisabled();
