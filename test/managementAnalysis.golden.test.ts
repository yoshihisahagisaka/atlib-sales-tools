...
test('SL-A5: read-only Human routes never start Assessment',async()=>{
 ...
 for(const route of ['DIRECT_ACT','FOCUSED_CONFIRMATION','DESIGN_ASSESSMENT','STOP_HOLD'] as const){
  ...
  assert.equal((await h.assessment.read(c.id,operator)).version,before.version);assert.equal((await h.assessment.read(c.id,operator)).assessment_status,'NOT_PROPOSED');
 }
});
