...
test('Human Review: AI提案→出典確認→承認・編集・UNKNOWN変換・却下→Assessment確認→完了',async({page},info)=>{
 ...
 await expect(page.locator('#company')).toHaveText('ABC株式会社様');await expect(page.getByText('IT経営KAIZEN 無料診断 / 提供：atLIB株式会社')).toBeVisible();await expect(page.locator('#future-status')).toContainText('SURVEY_STATED');
 await page.locator('#run-ai').click();await expect(page.locator('#executions')).toContainText('整理完了');await expect(page.locator('#case-status')).toContainText('HUMAN_REVIEW_REQUIRED');expect((await h.review.read(c.id,operator)).approved_insights).toHaveLength(0);
 ...
 await page.getByRole('button',{name:'担当者が確認して候補を作成',exact:true}).click();await page.getByRole('button',{name:'確認候補を保存',exact:true}).click();await expect(page.locator('#assessment-items')).toContainText('管理台帳の更新方法を確認する');await expect(page.locator('#assessment-items')).toContainText('OPEN');
 ...
 await page.locator('#leave-unreviewed').check();page.once('dialog',d=>d.accept());await page.locator('#complete').click();await expect(page.locator('#case-status')).toContainText('REPORT_REVIEW_REQUIRED');await expect(page.locator('#completion')).toContainText('operator@atlib.jp');await expect(page.locator('#reviews')).toContainText('APPROVE');await expect(page.locator('#reviews')).toContainText('operator@atlib.jp');await expect(page.locator('#run-ai')).toBeDisabled();await expect(page.locator('#editor')).toBeHidden();
...
test('Human-only Review: AI失敗→UNKNOWN作成→Supersedeで履歴保持→Review完了',async({page},info)=>{
 ...
 const before=await h.review.read(c.id,operator);await page.getByRole('button',{name:'内容を更新する',exact:true}).click();await page.locator('#insight-content').fill('担当者と役割の境界は未確認');await page.locator('#review-reason').fill('確認範囲を明確にする');await page.locator('#save-insight').click();await expect(page.locator('#approved')).toContainText('v2');
 await page.getByText('変更履歴',{exact:true}).click();await expect(page.locator('#history')).toContainText('SUPERSEDED');await expect(page.locator('#history')).toContainText('今は担当者を確認していない');await page.screenshot({path:info.outputPath('human-review-supersede.png'),fullPage:true});
 page.once('dialog',d=>d.accept());await page.locator('#complete').click();await expect(page.locator('#case-status')).toContainText('REPORT_REVIEW_REQUIRED');const report=await h.review.reportContext(c.id,operator);expect(report.insights).toHaveLength(1);expect(report.insights[0].previous_insight_id).toBe(before.approved_insights[0].id);expect(report.insights[0].semantic_type).toBe('UNKNOWN');
});
