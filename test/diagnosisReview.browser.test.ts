...
test('Human Review: AI提案→出典確認→承認・編集・UNKNOWN変換・却下→Assessment確認→完了',async({page},info)=>{
 ...
 await expect(page.locator('#company')).toHaveText('ABC株式会社様');await expect(page.getByText('IT経営KAIZEN 無料診断 / 提供：atLIB株式会社')).toBeVisible();await expect(page.locator('#future-status')).toHaveText('SURVEY_STATED');
 await page.locator('#run-ai').click();await expect(page.locator('#executions')).toContainText('整理完了');await expect(page.locator('#case-status')).toHaveText('HUMAN_REVIEW_REQUIRED');expect((await h.review.read(c.id,operator)).approved_insights).toHaveLength(0);
