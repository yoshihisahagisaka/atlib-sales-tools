...
test('Workspace: Human Start → 原文記録 → AI候補のAsk/Later/Unnecessary → 未確認を残してFinish',async({page},info)=>{
 ...
 await expect(page.locator('#company')).toHaveText('ABC株式会社様');await expect(page.getByText('IT経営KAIZEN 無料診断 / 提供：atLIB株式会社')).toBeVisible();
 await expect(page.locator('#run-ai')).toBeDisabled();await page.locator('#start').click();await expect(page.locator('#case-status')).toHaveText('確認・分析中');
 ...
 const before=await h.workspace.read(c.id,operator);await page.locator('#run-ai').click();await expect(page.locator('#executions')).toContainText('整理完了');
 await expect(page.locator('#suggestions .suggestion-card')).toHaveCount(5);
 const cards=page.locator('#suggestions .suggestion-card');await cards.nth(0).getByRole('button',{name:'今回確認する',exact:true}).click();await expect(cards.nth(0)).toContainText('判断：今回確認する');
 await cards.nth(1).getByRole('button',{name:'後で確認する',exact:true}).click();await expect(cards.nth(1)).toContainText('判断：後で確認');
 await cards.nth(2).getByRole('button',{name:'今回は確認しない',exact:true}).click();await expect(cards.nth(2)).toContainText('判断：今回は確認しない');
 await cards.nth(0).getByText('もとになった情報を見る',{exact:true}).click();
 const proposal=(await h.workspace.read(c.id,operator)).proposals[0];
 const source=before.sources.find(s=>s.id===proposal.content_json.source_refs[0].source_ref_id);
 await expect(cards.nth(0).locator('details')).toContainText(source.content);
 await expect(cards.nth(0).locator('details')).not.toContainText(source.id);
 ...
 page.once('dialog',d=>d.accept());await page.locator('#finish').click();await expect(page.locator('#case-status')).toHaveText('分析内容の確認待ち');await expect(page.locator('#run-ai')).toBeDisabled();await expect(page.locator('#statement-form')).toBeHidden();
...
test('Future再確認・Transcript追跡・自発的Evidence存在とHuman追加',async({page})=>{
 ...
 await page.goto(`${h.url}/admin/it-management-diagnosis-workspace.html?id=${c.id}`);await page.locator('#start').click();await expect(page.locator('#case-status')).toHaveText('確認・分析中');
 await page.getByText('会話記録を貼り付けて保存',{exact:true}).click();await page.locator('#transcript-content').fill('顧客：採用を増やせる会社を目指す');await page.getByRole('button',{name:'会話記録を保存',exact:true}).click();await expect(page.locator('#sources')).toContainText('会話記録');
 const transcript=(await h.workspace.read(c.id,operator)).sources[0];await page.locator('#statement-parent').selectOption(transcript.id);await page.locator('#statement-content').fill('採用を増やせる会社を目指す');await page.getByRole('button',{name:'顧客発言を保存',exact:true}).click();await expect(page.locator('#sources')).toContainText('もとになった情報を見る');
 await page.getByText('対話で目指している会社の姿を再確認する',{exact:true}).click();await page.locator('#future-statement').fill('採用を増やせる会社を目指す');const statement=(await h.workspace.read(c.id,operator)).sources.find(s=>s.source_type==='INTERVIEW_STATEMENT');await page.locator('#future-source').selectOption(statement.id);
 page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'担当者として会社の姿を再確認したと記録',exact:true}).click();await expect(page.locator('#future-status')).toHaveText('顧客の発言として再確認');
 await page.getByText('目指している会社の姿の記録',{exact:true}).click();await expect(page.locator('#future-history')).toContainText('顧客の回答として記録');
 await page.getByText('資料等の存在を記録',{exact:true}).click();await page.locator('#evidence-content').fill('台帳が存在することを画面共有で確認した');await page.locator('#evidence-voluntary').check();await page.getByRole('button',{name:'資料等の存在を保存',exact:true}).click();await expect(page.locator('#sources')).toContainText('資料等の存在を確認');
 ...
 page.once('dialog',d=>d.accept());await page.locator('#finish').click();await expect(page.locator('#case-status')).toHaveText('分析内容の確認待ち');
...
test('AI failureでもHuman-onlyの記録・終了が可能',async({page},info)=>{
 ...
 await page.screenshot({path:info.outputPath('workspace-human-only.png'),fullPage:true});page.once('dialog',d=>d.accept());await page.locator('#finish').click();await expect(page.locator('#case-status')).toHaveText('分析内容の確認待ち');
});
