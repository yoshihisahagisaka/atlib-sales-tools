...
export async function createDiagnosisHarness(notify?: CompletionNotifier) {
  ...
  app.use('/api/admin/it-management-diagnosis', requireStaffAuth(staffAuth), createAdminItManagementDiagnosisRouter(repo, notify));
  ...
  return { db, pool, repo, url, logs, staffCookie,
