...
test('7–14: approved semantic types and source refs preserved; separate OPEN confirmation items; no raw duplication',async()=>{
 ...
 // Explicit Human return to Review to prepare all canonical semantic types and history.
 ...
 const s=await snapshot(c.id),approved=(await h.review.read(c.id,operator)).approved_insights as Array<{id:string;semantic_type:string;review_status:string;source_refs:unknown;content:string;unknown_type:string|null}>;
 assert.equal(s.insights.length,approved.length);assert.deepEqual(new Set(s.insights.map(i=>i.id)),new Set(approved.map(i=>i.id)));for(const i of s.insights){const original=approved.find(x=>x.id===i.id)!;assert.equal(i.semantic_type,original.semantic_type);assert.equal(i.review_status,'HUMAN_APPROVED');assert.deepEqual(i.source_refs,original.source_refs);assert.equal(i.content,original.content);}
 assert.ok(s.insights.some(i=>i.id===replacement.id&&i.unknown_type==='UNRESOLVED'));assert.equal(s.insights.some(i=>i.id===c.unknown.id),false);assert.ok(s.insights.some(i=>i.semantic_type==='EVIDENCE_CANDIDATE'));
 const openItems=(await h.db.query("SELECT id FROM assessment_confirmation_items WHERE diagnosis_case_id=$1 AND status='OPEN' ORDER BY id",[c.id])).rows as Array<{id:string}>;assert.equal(s.assessment_confirmation_items.length,openItems.length);assert.deepEqual(new Set(s.assessment_confirmation_items.map(i=>i.id)),new Set(openItems.map(i=>i.id)));for(const i of s.assessment_confirmation_items)assert.equal('semantic_type' in i,false);
