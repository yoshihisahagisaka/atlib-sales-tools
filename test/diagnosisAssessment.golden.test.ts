...
test('7–14: approved semantic types and source refs preserved; separate OPEN confirmation items; no raw duplication',async()=>{
 ...
 const openItems=(await h.db.query("SELECT id FROM assessment_confirmation_items WHERE diagnosis_case_id=$1 AND status='OPEN' ORDER BY priority,created_at,id",[c.id])).rows.map(x=>x.id);
 assert.ok(s.insights.some(i=>i.id===replacement.id&&i.unknown_type==='UNRESOLVED'));assert.equal(s.insights.some(i=>i.id===c.unknown.id),false);assert.ok(s.insights.some(i=>i.semantic_type==='EVIDENCE_CANDIDATE'));assert.deepEqual(s.assessment_confirmation_items.map(i=>i.id),openItems);assert.equal(s.assessment_confirmation_items.some((i:any)=>'semantic_type' in i),false);
